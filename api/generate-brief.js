...
export default async function handler(req, res) {
  ...
  if (quota && quota.ok === false) {
    ...
    const msg = lim === 0
      ? 'البريف بالذكاء الاصطناعي متاح مع باقة Standard فأعلى. باقتك الحالية: ' + (quota.plan || 'Basic') + '.'
      : 'استهلكت حصة هذا الشهر من البريف الذكي (' + lim + '). تتجدد أول الشهر القادم، أو ارتقِ لباقة Premium للاستخدام بلا حد.';
  ...
  note = String(note || '').trim().slice(0, 1500);
  if (!title) return res.status(400).json({ error: 'اكتب عنوان الحملة أولاً عشان أقترح لك البريف' });
  if (!note) return res.status(400).json({ error: 'اكتب باختصار وش تبي من البريف عشان يكون قريب لتصوّرك' });
...
القواعد:
- **الأهم:** "طلب الشركة" في الأسفل هو جوهر البريف — التزم به حرفيًا، وسّعه ونظّمه ولا تخالفه ولا تتجاهل أي نقطة ذكرتها الشركة. باقي الحقول (العنوان/التصنيف/المنصة) سياق مساعد فقط.
- لا تخترع تفاصيل تخالف طلب الشركة. لو الشركة ما ذكرت شيئًا لقسم معيّن، استنتج استنتاجًا منطقيًا بسيطًا متسقًا مع طلبها، أو اجعله عامًا مختصرًا.
...
- لا تخترع أرقام أسعار أو مواعيد أو أكواد خصم. لا تذكر سعر ولا ميزانية ولا عمولة إطلاقًا.
- إجمالي البريف بين 110 و 200 كلمة تقريبًا.
- أخرج نص البريف فقط، بدون عناوين ماركداون ولا رموز نجمية ولا تعليق منك.`;

  const userPrompt = `طلب الشركة (وش تبي من الإعلان والمعلومات الأساسية) — هذا الأساس:
"""
${note}
"""
...
- منصة النشر: ${platformLabel}

اكتب الآن البريف الإعلاني الكامل بناءً على طلب الشركة أعلاه.`;
